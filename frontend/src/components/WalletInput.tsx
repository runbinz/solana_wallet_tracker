...
export default function WalletInput({ onSubmit, isLoading, recentWallets, onPickRecent }: WalletInputProps) {
  ...
  return (
    <motion.div
      ...
      className="rounded-md glass shadow-card overflow-hidden"
    >
      ...
      <div className="p-6 sm:p-8">
        <div className="mb-6">
          <p className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-accent-purple">
            Wallet lookup
          </p>
          ...
          <div className="mb-5 flex flex-wrap items-center gap-2">
            ...
            {recentWallets.map((w, i) => (
              <motion.button
                ...
                className="max-w-[180px] truncate rounded-sm border border-[var(--nav-border)] bg-[var(--surface-soft)] px-3 py-1.5 font-mono text-xs text-[var(--text-secondary)] transition-all duration-200 hover:-translate-y-0.5 hover:bg-[var(--surface-strong)] hover:text-[var(--text-primary)] hover:shadow-sm disabled:opacity-50"
                title={w}
              >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:flex-row">
          <div className="relative flex-1 group">
            ...
            <button
              ...
              className="absolute right-3 top-1/2 -translate-y-1/2 rounded-sm p-1.5 text-[var(--text-muted)] opacity-60 transition-all hover:bg-[var(--surface-soft)] hover:text-accent-purple hover:opacity-100"
          ...
          <motion.button
            ...
            className="relative flex items-center justify-center gap-2.5 overflow-hidden rounded-md px-8 py-4 font-semibold text-white shadow-lg shadow-accent-purple/20 transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-50 solana-gradient hover:shadow-xl hover:shadow-accent-purple/30"
          >
